...
| Real Liquid Glass, from real controls | native `UITabBar` (`AdaptiveNavigationScaffold`) and `UISegmentedControl` (`AdaptiveSegmentedControl`) on glass eras. `UIVisualEffectView` cannot sample Flutter's `CAMetalLayer`, so the material has to come from the system control itself |
| Transient glass on sliders and toggles | native `UISwitch` / `UISlider` on glass eras — the system applies it during the gesture |
...
2. **`sidebarAdaptable` toggle** — the button that converts an iPad tab bar into
   a sidebar and back. Today the choice is fixed at build time by
   `AdaptiveNavigationStyle`.
3. **Concentric corner radii** for components embedded in bars. Apple publishes
   no value, so this has to be derived from the bar's own radius.
4. **Search placements** — three on iOS, a different set on iPadOS/macOS, plus
   the iPad virtual-keyboard auto-focus exception. No adaptive search widget
   exists yet.
5. **Toolbar item regions** — leading / center / trailing with a system-managed
   overflow and one `.prominent` trailing action. `AdaptiveScaffold` still models
   the iOS 18 leading/middle/trailing shape.
6. **Tab bar accessory** — the MiniPlayer-style view that makes minimising
   meaningful.
7. **Native navigation bar.** `CupertinoNavigationBar` still draws the iOS 18
   bar. The same argument as the tab bar applies — a real `UINavigationBar` would
   carry the system material — but it also owns the title, back button and
   toolbar items, so embedding it is a larger change than swapping a control.
...
12. **iOS 27 era.** Apple's design resources already ship an iOS 27 / iPadOS 27
    ...
    covered today; a separate era is needed the moment 27 diverges. See the
    appendix below for what a community kit says has actually changed.

---

## Appendix: iOS 27 changes seen in a community Figma kit (unofficial)

**Not an Apple source.** Everything below comes from the changelog of a
community-maintained Figma file, [iOS and iPadOS 27
(Community)](https://www.figma.com/design/UTmD4NWpDuRbgDyCbbxzzv/iOS-and-iPadOS-27--Community-),
read on 23 August 2026 by scrolling its "Change log" board. It is not Apple
documentation, has no changelog authority the way the dated HIG pages do, and
is kept separate from [Numbers Apple actually
publishes](#numbers-apple-actually-publishes) for that reason — nothing here
should be treated as a published Apple figure. Recorded so a future session
does not have to re-scroll the same board.

**27.0.3** (12 August) — Sheets: changed iPad sheet toolbar button insets from
16pt to 10pt; changed iPad sheet corner radius from 38pt to 32pt.

**27.0.2** (16 July) — Tab bars: fixed several unlinked Light/Dark mode
variable bindings (5th tab, backgrounds).

**27.0.1** (24 June) — Sheets: updated linking to new sheets.

**27.0.0** (23 June) — the largest entry, by section:
- *General*: removed remaining links to the Liquid Glass appearance-mode
  collection — i.e. the kit no longer models glass as a togglable appearance,
  consistent with this package's own view that Liquid Glass is load-bearing on
  iOS 26+, not optional.
- *Colors*: removed increased-contrast mode, citing limited subcomponent
  support interfering with light/dark appearance switching.
- *Materials*: updated the appearance of all Liquid Glass components; updated
  soft- and hard-style Scroll Edge Effects; changed the "State" property of
  the "Liquid Glass - Regular" component from a named state to a boolean
  "Prominent" — a components-file simplification, not a new visual token.
- *Action sheets*: converted the buttons frame to a slot; renamed
  "Description" to "Message"; removed a 10pt gap between title/message and
  actions.
- *Activity views*: updated font sizes and button/list-item layout; fixed a
  separator not switching to dark mode.
- *Alerts*: converted the button frame to a slot; renamed "Description" to
  "Message"; updated spacing between stacked and side-by-side buttons;
  removed extra padding between message, text field, and buttons; updated
  alert-button weight from Medium to Semi-Bold.
- *Buttons*: renamed the "Tinted" style to "Style" with values "Glass" and
  "Glass Prominent" — the kit's own naming lines up with
  `UIButton.Configuration.glass()` / `.prominentGlass()`, which is exactly the
  API `GlassButtonFactory` already targets; added disabled states for every
  glass button style; added small/regular/large size variants for glass
  buttons (no dp values recorded — the kit exposes these as component
  variants, not published numbers); renamed "State" to "Is Enabled"
  (true/false); removed secondary buttons as a distinct style.
- *Color pickers*: placed the iPhone color picker in a medium-detent sheet
  with a full-screen overlay; assorted small layout/size updates.
- *Date and time pickers*: adjusted calendar-day size, collapsed
  date/time-picker height, selected-date appearance, and date/time-picker
  layout; changed the color of the next/previous [month controls, cut off].
- *Keyboards*: changelog continues past this point but was not transcribed —
  re-open the Figma link above and scroll past "Keyboards" for tab bars,
  navigation bars, search, segmented controls, sheets, sliders, switches, and
  text field entries, none of which were captured this session.

**What this changes about the "skip iOS 27" decision above:** nothing yet.
27.0.0's "Materials" and "Buttons" entries describe refinements to the same
glass vocabulary this package already implements (`GlassSurfaceFactory`,
`GlassButtonFactory`'s `.glass()`/`.prominentGlass()`), not a divergent one —
so the `>= 26` threshold still holds. The one concrete, actionable number is
the 27.0.3 iPad sheet delta (corner radius 38pt→32pt, toolbar insets
16pt→10pt), relevant only if/when this package grows an `AdaptiveSheet`
component for iPad.
